...
(function(jq){
	...
	posterTvGrid.prototype = {
		reset: function(options){
			...
			this.preLeft = 0;
			this.nextLeft = this.options.width-200;
			this.preNLeft = -200;
			this.nextNLeft = this.options.width;
			this.pageNowLeft = (this.options.width-296)/2
			this.drawContent();
		},
		...
		initContent: function(){
			...
			this.parent.find(".contentHolderUnit:nth-child("+this.pageNow+")").css({width:'296px',height:'354px', opacity: 1, left: this.pageNowLeft+'px', zIndex: 3, marginTop: 0});
			...
			this.parent.find(".contentHolderUnit:nth-child("+pre+")").css({opacity: 1, left: this.preLeft+'px',height: '224px', width: '200px', zIndex: 0, marginTop: '23px'});
			...
			this.parent.find(".contentHolderUnit:nth-child("+next+")").css({opacity: 1, left: this.nextLeft+'px',height: '224px', width: '200px', zIndex: 0, marginTop: '23px'});
		...
		turnpage: function(page, dir){
			...
			var run = function(page, dir, t){
				...
				if(dir == 'left'){					
					...
					_this.parent.find(".contentHolderUnit:nth-child("+pre+")").animate({opacity: 1, left: _this.preLeft+'px', height: '239px', width: '200px', zIndex: 2, marginTop: '23px'}, t);
					...
					_this.parent.find(".contentHolderUnit:nth-child("+page+")").animate({opacity: 1, left: _this.pageNowLeft+'px', height: '354px', width: '296px', zIndex: 3, marginTop: '0'}, t);
					...
					_this.parent.find(".contentHolderUnit:nth-child("+next+")").animate({opacity: 1, left: _this.nextLeft+'px', height: '239px', width:"200px", zIndex: 2, marginTop: '23px'}, t);
					...
					_this.parent.find(".contentHolderUnit:nth-child("+preP+")").animate({width:'200px',height:'100px', opacity: 0, left:_this.preNLeft+'px', zIndex:0, marginTop: '85px'},t, "", function(){_this.locked=false;});
					...
					_this.parent.find(".contentHolderUnit:nth-child("+page+")").animate({opacity: 1, left: _this.pageNowLeft+'px', height: '354px', width: '296px', zIndex: 3, marginTop: '0px'}, t);
					_this.parent.find(".contentHolderUnit:nth-child("+page+") .elementOverlay").css({opacity:0});
					_this.parent.find(".contentHolderUnit:nth-child("+page+") .leftShadow").css({opacity:1});
					_this.parent.find(".contentHolderUnit:nth-child("+page+") .rightShadow").css({opacity:1});
					
					_this.parent.find(".contentHolderUnit:nth-child("+pre+")").css({opacity: 0, left: _this.preNLeft+'px', height: '100px', width: '200px', zIndex: 2, marginTop: '85px'});
					_this.parent.find(".contentHolderUnit:nth-child("+pre+")").animate({opacity: 1, left: _this.preLeft+'px', height: '239px', width: '200px', zIndex: 2, marginTop: '23px'}, t);
					_this.parent.find(".contentHolderUnit:nth-child("+pre+") .elementOverlay").css({opacity:0.4});
					_this.parent.find(".contentHolderUnit:nth-child("+pre+") .leftShadow").css({opacity:0});
					_this.parent.find(".contentHolderUnit:nth-child("+pre+") .rightShadow").css({opacity:0});
					...
					_this.parent.find(".contentHolderUnit:nth-child("+nextN+")").animate({width:'200px',height:'100px', opacity: 0, left:_this.nextNLeft+'px', zIndex:0, marginTop: '85px'}, t, "",function(){_this.locked=false;});
				}
	...
	posterTvGrid.options = {
		offsetPages : 5,//榛樿鍙鏈€澶ф潯鏁�
		direct : "left",//婊氬姩鐨勬柟鍚�
		initPage : 1,//榛樿褰撳墠鏄剧ず绗嚑鏉�
		className : "posterTvGrid",//鏈€澶栧眰鏍峰紡
		autoWidth : true,//榛樿涓嶇敤璁剧疆瀹�
		width : 970,//鏈€澶栧眰瀹斤紝闇€瑕佷娇鐢ㄧ殑鏃跺€欏湪浼�,榛樿鐢辩▼搴忚嚜鍔ㄥ垽鏂�
		height : 374,//鏈€澶栧眰楂�  
		delay : 5000,//婊氬姩闂撮殧锛堟绉掞級
		speed : 500 //婊氬姩閫熷害姣
	};
